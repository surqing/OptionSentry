...
class GuiSmokeTests(unittest.TestCase):
    def test_pyqt_login_window_constructs_offscreen(self) -> None:
        ...
        self.assertTrue(main_window.config_editor.exclude_do.isHidden())
        main_window.config_editor.universe_mode.setCurrentText("onlyDo")
        app.processEvents()
        ...
        main_window.config_editor.universe_mode.setCurrentText("excludeDo")
        app.processEvents()
        self.assertTrue(main_window.config_editor.exchange_ids.isHidden())
        self.assertTrue(main_window.config_editor.only_do.isHidden())
        self.assertFalse(main_window.config_editor.exclude_do.isHidden())
        main_window.config_editor.exclude_do.setPlainText("Cu")
        self.assertEqual(main_window.config_editor.build_config().universe.exclude_do, ("CU",))
        ...
        self.assertEqual(main_window.alert_table.item(0, 1).text(), "CP组合预警")

        main_window._on_alert(_alert_event("2026-06-26 23:42:05", strategy_name="价差预警"))
        self.assertEqual(main_window.alert_view.filter_labels(), ("全部策略", "CP组合预警", "价差预警"))
        self.assertEqual(main_window.alert_table.rowCount(), 2)
        main_window.alert_view.set_strategy_filter("CP组合预警")
        ...
        main_window._on_cycle(
            RunnerCycle(
                ...
                evaluations=(
                    _evaluation(strategy_name="CP组合预警"),
                    _evaluation(strategy_name="价差预警"),
                ),
                total_conditions=0,
                active_count=2,
                alerts=(),
...
def _alert_event(timestamp: str, strategy_name: str = "CP组合预警", value: float = 1.0) -> AlertEvent:
    return AlertEvent(
        timestamp=timestamp,
