...
GitHub Actions 中的 [release.yml](.github/workflows/release.yml) 会在推送 `v*.*.*` 标签或手动触发时构建 Windows 包。发布标签需要和 [pyproject.toml](pyproject.toml) 中的 `version` 保持一致，例如 `version = "0.1.1"` 对应标签 `v0.1.1`。
